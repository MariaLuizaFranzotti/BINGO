...
function resetGame() {
    ...
    document.querySelector('#winner').textContent = ""; // resetar texto do ganhador
    players = [];
    drawnNumbers = [];
}
...
function drawNumber() {
    ...
    reloadCards()
    // verifica se há um vencedor após cada número ser sorteado
    players.forEach(player => {
        if (checkWinner(player)) {
            // se encontrar um vencedor, exiba seu nome e pare o jogo
...
document.querySelector('#play-btn').addEventListener('click', startGame); // Corrigi o nome do botão aqui
